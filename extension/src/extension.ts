...
export function activate(context: vscode.ExtensionContext) {
    try {
        ...
        log("GenAI Studio Preview extension activated");
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand(
                ...
                async (item?: ProjectItem) => {
                    ...
                    const project = projectsTreeProvider.getProject(
                        item.projectPath,
                    );
                    if (project) {
            ...
            vscode.commands.registerCommand(
                "genai-preview.addProject",
                async () => {
                    try {
                        log("Add Project command triggered");
                        ...
                        log(`Adding project: ${projectName} at ${projectPath}`);
                        await projectsTreeProvider.addProject(
                            projectName,
                            projectPath,
                        );
                        refreshAll();
                        log(`Project added successfully: ${projectName}`);
                    } catch (error) {
            ...
            vscode.commands.registerCommand(
                ...
                async (item?: ProjectItem) => {
                    ...
                    if (confirm === "Remove") {
                        await projectsTreeProvider.removeProject(
                            item.projectPath,
                        );
                        refreshAll();
                    }
        ...
        vscode.window.showErrorMessage(
            `GenAI Preview Activation Failed: ${
